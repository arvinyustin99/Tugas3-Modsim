...
public class MachineShop extends SimulationProcess {
  ...
  public void run() {
    try {
      ...
      System.out.println("Total number of jobs present (L) " + rtime.numberOfSamples());
      ...
      System.out.println("Average response time (L) = " + rtime.mean());
      System.out.println("Average response time = " + (TotalResponseTime / ProcessedJobs));
      System.out.println(
          "Probability that machine is working = " + ((MachineActiveTime - MachineFailedTime) / CurrentTime()));
  ...
  public static Mean rtime = new Mean();
